...
def check_fasta(a):
    dir = 'original_datasets/33_riboswitches_fasta/'
    with open(dir+a,"r") as f:
        f=f.readlines()
        i=0
        list=[]
        while i < len(f):
...
def makecsv(file, dir='original_datasets/33_riboswitches_fasta/'):
    input_file = open(dir+file, 'r')
    ...
    try:
        ...
        if args.d is not None:
            for eachfile in os.listdir(args.d):
                print(eachfile)
                makecsv(eachfile, args.d)
                csvfiles = 'original_datasets/33_riboswitches_new_csv'
                csv="{}.csv".format(eachfile.split(".")[0])
                final_csv="processed_datasets/final_dynamic.csv"
                output = open(final_csv,'r')
                lastclass=output.readlines()
                if len(lastclass) == 0:
                    lastclass = -1
                elif len(lastclass) == 1:
                    lastclass = -1
                else:
                    lastclass=int(lastclass[-1].split(",")[1])
                output.close()
...
#add headers columns to final_dynamic.csv before starting      Sequence,Type,A,T,G,C,AA,AC,AG,AT,CA,CC,CG,CT,GA,GC,GG,GT,TA,TC,TG,TT
