...
ALLOWED_ALPHABETS = 'ATGCNU' # Allowed Charecters ajoute nucleotide U

def processingscript():
    subprocess.call(['mkdir','original_datasets/33_riboswitches_new_csv'])
    def makecsv(file):
        input_file = open(file, 'r')
        csvfiles = 'original_datasets/33_riboswitches_new_csv'
        file=file.split("/")
        filename=file[2]
    ...
    nu_of_genes=[]
    for i in os.listdir('original_datasets/33_riboswitches_new_csv'):

        g=subprocess.Popen(['wc','-l','original_datasets/33_riboswitches_new_csv/{}'.format(i)],stdout=subprocess.PIPE)
        g=g.stdout.read()
        g=g.decode("utf-8")
        g=g.strip("\n")
    ...
    f=open("count.csv",'w+')
    k=0
    for i,j in zip(os.listdir('original_datasets/33_riboswitches_new_csv'),nu_of_genes):
        print(i,j)
        j=(j.strip(" "))
        j=(j.split(" ")[0])
    ...
    final_csv="processed_datasets/final_33classes.csv"
    output = open(final_csv,'w+')

    type=0
    for j in os.listdir('original_datasets/33_riboswitches_new_csv'):
        dir='original_datasets/33_riboswitches_new_csv'
...
def split_dataset(Path = 'processed_datasets/final_33classes.csv'):
    ...
    file_name = 'processed_datasets/final_33train.csv'
    Data_train.to_csv(file_name, sep=',', encoding='utf-8', index=False)
    file_name = 'processed_datasets/final_33test.csv'
...
if __name__ == '__main__':
    split_dataset('processed_datasets/final_dynamic.csv')