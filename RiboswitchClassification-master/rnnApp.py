...
ALLOWED_ALPHABETS = 'ATGCNU' # Allowed Charecters add U nucleotide
INPUT_DIM = len(ALLOWED_ALPHABETS) # a vocabulary of 5 words in case of genome sequence 'ATGCN'
CLASSES = 33 # Number of Classes to Classify -> Change this to 16 when needed
...
checkpoint_dir ='epoch_tuning/RNN/33_checkpoints'
os.path.exists(checkpoint_dir)

# Path to save and load Model 
model_file_h5 = "models/rnn_33_model.h5"

# Path to Dataset
input_file_train = 'processed_datasets/final_33train.csv'
input_file_test  = 'processed_datasets/final_33test.csv'
...
def create_lstm(input_length, rnn_hidden_dim = RNN_HIDDEN_DIM, output_dim = OUTPUT_DIM, input_dim = INPUT_DIM, dropout = DROPOUT_RATIO):
    ...
    model.compile('adam', 'categorical_crossentropy', metrics=['accuracy']) # binary_crossentropy # categorical_crossentropy # sparse_categorical_crossentropy
    return model
...
def generate_auc_roc(X_test, y_test):
    ...
    score, acc = model_loaded.evaluate(X_test, to_categorical(y_test),batch_size=BATCH_SIZE)
    print (score)
    print (acc)
    ...
    # y_score = np.delete(y_score, np.s_[0:2], axis=1)
    nb_classes = 33
    multiclassROC.calculate_roc(bin_output, y_score, "RnnClassifierModel", nb_classes)
...
if __name__ == '__main__':
    ...
    model.summary()
    # # Load Test Datasets
    X_test, y_test = preprocess.load_data(input_file_test, True)
    # # Train Model and Save it
    model = train_model_and_save(X_train, y_train, model)
    # Generate Auc and Roc Curve
    generate_auc_roc(X_test, np.argmax(y_test, axis=1))
